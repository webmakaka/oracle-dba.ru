...
UPD! Если вы работаете в Linux, то базу можно запустить парой команд  <a href="/database/installation/single-instance/simple/linux/docker/oracle/11/xe/">в docker контейнере</a>. Под Windows докер тоже работает, но там все (для меня) сложнее. Сложнее в плане настройки сети и ненужной прослойки в виде VirtualBox.
